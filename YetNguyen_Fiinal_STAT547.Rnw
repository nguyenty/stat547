...
\section{Functional Linear Model with Functional Covariate as Temperature and Scalar Response as Log of Total Precipitation}
...
@
\subsection{Ramsay and Silverman's Approach}
...
\subsection{Principle Component Analysis Approach}
...
plot(y = fregre.cv.out$fitt, x = prectotal, 
     ...
     main = "Prediction by Ramsay Silverman's approach",
     ylab = "Predicted Values")
abline(a=0,b=1)
sqrt(mean(fregre.cv.out$fitt- prectotal)^2)
fregre.pc.cv.out <- fregre.pc.cv(aemet$temp, prectotal, kmax = 30, lambda = TRUE)

plot(y = fregre.pc.cv.out$fitt, x = prectotal, 
     xlab ="log total precipitation", 
     main = "Prediction by Principle Component Analysis approach",
     ylab = "Predicted Values")
abline(a=0,b=1)
sqrt(mean(fregre.pc.cv.out$fitt- prectotal)^2)
